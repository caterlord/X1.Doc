...
`已发布` 和 `公开显示` 是两项不同的检查。`已发布` 是分类／门店安排的可用状态；`公开显示` 控制分类是否向顾客显示。在 `Shop Display Settings` 中编辑门店，使用 `发布` 开关更新门店的公开状态；保存后，该门店会保持启用和公开。

将分类加入线上点餐，或建立新的线上门店映射时，HQ 会默认新映射为已发布／公开。如果分类不应在每家门店显示，请保存前逐一检查门店。
...
11. 在 `Shop Display Settings` 中检查每家门店的 `已发布` 值；需要改变顾客可见状态时，选择 `Edit` -> `发布`。
...
3. 在相关门店的 `Shop Display Settings` 再检查 `已发布`。
4. 查看相关详细分页，确认预期商品、门店设置或渠道已经连结。
...
- 如果 HQ 显示分类但线上看不到，请同时检查分类 `已发布` 和门店的 `公开显示`／`已发布` 状态。
