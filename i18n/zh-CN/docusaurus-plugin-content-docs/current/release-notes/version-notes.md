...
- `第三方平台`：补充 Foodpanda 的商户优先表单、可搜索 ID 选择器、`无法使用的 ID（{id}）`、项目选择器、保存后同步要求，以及手动 `菜单同步`。
...
- 更新 POS 报表待补截图，要求显示 `14. 整单取消报表` 及其审计栏位。
...
- `设置` 和 `功能与特性地图`：补充模板字体设置现在包含 `八达通增值单` 和 `入账 / 出账单`，这些单据可按已保存的字号设置输出。
