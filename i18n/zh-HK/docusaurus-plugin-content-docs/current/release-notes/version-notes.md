...
- `菜單項目`：補充每間店舖的 `外賣附加費` 和可展開的 `其他設定`。
- `智能分類`、`網上分類` 和 `網上菜單`：補充 `已發佈`、`公開顯示`、`顯示未發佈`、`顯示於 ODO`，以及新店舖對應的預設發佈狀態。
- `第三方平台`：補充 Foodpanda 的商戶優先表單、可搜尋 ID 選擇器、`無法使用的 ID（{id}）`、項目選擇器、儲存後同步要求，以及手動 `菜單同步`。
- `折扣`：補充標準 `折扣：百分比` 保留價格倍率和 `套用到商品層級` 行為。

### 截圖更新

- 新增 POS 儀表板圖表、列印預覽和 `需要處理` 動作的待補截圖。
- 新增 HQ 外賣附加費、ODO 發佈狀態、折扣百分比輸入、網上菜單店舖時段、網上分類發佈狀態和 Foodpanda 菜單同步的待補截圖。
- 保留 MeiTuan Webhook 截圖要求，並與新的第三方平台指引連結。

### 已檢查

- 已將 POS tag `v1.8.0`（`ce1b6414`）與 `v1.7.19..v1.8.0` 來源範圍對照；印表機流程及報表儀表板是需要新增公開步驟的變更。
- 已檢查 PAX 分拆／合併穩定性、帳單折扣重新驗證，以及 POS 的執行環境、網絡、診斷、當機和封裝變更；這些不新增現有指引以外的公開復原步驟。
- 已檢查 HQ 2026-07-18 基線後至 `8ae01e6` 的商戶功能變更，包括付款方式、店舖附加費、網上發佈、Foodpanda／菜單同步和折扣設定。
