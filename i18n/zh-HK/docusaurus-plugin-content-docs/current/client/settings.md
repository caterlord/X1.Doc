...
僅限管理員。請先準備標籤紙尺寸與印表機的網路位址。

1. 開啟 `設定` -> `列印` -> `模板`。
2. 將 `模板` 設為 `標籤`。
3. 按照標籤紙設定 `紙張寬度（毫米）` 和 `紙張高度（毫米）`。新設定預設為 40 × 30 毫米；既有設定保留已儲存的寬度。
4. 將 `標籤間距（毫米）` 設為兩張標籤之間的空隙。預設為 3 毫米；連續紙請設為 0。
5. 點選 `儲存`。
6. 開啟 `裝置`，編輯網路標籤印表機。
7. TSC DA210 或相容的 203 dpi TSPL/TSPL2 印表機請選擇 `TSPL 標籤（203 dpi）`，確認 IP 位址與連接埠後儲存。
8. 使用 `測試列印`，印出一張帶邊框的標籤。確認邊框位於紙張內，下一張標籤從間隙後開始。

文字會按紙張寬度自動換行，每個品項的紙張尺寸保持一致。如果文字超出高度，請在同一模板中開啟 `啟用自適應文字大小` 並儲存。系統會先換行，再視高度縮小文字。如果仍有內容被裁切，請使用較大的標籤或減小本文字體。

此驅動程式使用網路列印連線，也可搭配相容的網路列印伺服器。不支援 300 dpi 印表機或直接 USB／藍牙連線。如果標籤跳紙或位置偏移，請確認紙張尺寸與間距，再請管理員校準印表機的間隙感應器。預覽不能確認實際對位。
