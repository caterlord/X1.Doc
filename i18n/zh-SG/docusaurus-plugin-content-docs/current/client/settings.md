...
仅限管理员。请先准备标签纸尺寸与打印机的网络地址。

1. 打开 `设置` -> `打印` -> `模板`。
2. 将 `模板` 设为 `标签`。
3. 按照标签纸设置 `纸张宽度（毫米）` 和 `纸张高度（毫米）`。新设置默认为 40 × 30 毫米；已有设置保留已保存的宽度。
4. 将 `标签间距（毫米）` 设为两张标签之间的空隙。默认为 3 毫米；连续纸请设为 0。
5. 点击 `保存`。
6. 打开 `设备`，编辑网络标签打印机。
7. TSC DA210 或兼容的 203 dpi TSPL/TSPL2 打印机请选择 `TSPL 标签（203 dpi）`，确认 IP 地址与端口后保存。
8. 使用 `测试打印`，打印一张带边框的标签。确认边框位于纸张内，下一张标签从间隙后开始。

文字会按纸张宽度自动换行，每个商品的纸张尺寸保持一致。如果文字超出高度，请在同一模板中开启 `启用自适应文字大小` 并保存。系统会先换行，再根据高度缩小文字。如果仍有内容被裁切，请使用较大的标签或减小正文字体。

此驱动程序使用网络打印连接，也可搭配兼容的网络打印服务器。不支持 300 dpi 打印机或直接 USB／蓝牙连接。如果标签跳纸或位置偏移，请确认纸张尺寸与间距，再请管理员校准打印机的间隙传感器。预览不能确认实际对位。
