...
## 2026-06-10（HQ 商户后台文档更新）

### 已更新

- `仪表板`：补充空工作区 onboarding 提示、引导式设置、当前指标卡、日期控件，以及仪表板表现区块。
- `Insights`：新增 HQ 指南，说明跨店铺发现、预设筛选、日期比较、图表 drilldown 和表格查看。
- `报表`、`销售报表` 和 `营运报表`：补充当前报表导航、导出动作、销售趋势视图、付款／桌号／堂食及外卖分析、出勤和员工成本效率。
- `智能分类`、`工作区管理` 和 `HQ 用户手册`：将智能分类管理、工作区／成员指引、onboarding 顺序和侧边栏结构，对齐已部署的商户 HQ。

### 截图更新

- 新增或替换 HQ 截图，涵盖空工作区 onboarding、引导式工作区设置、`Insights`、`智能分类`、`销售报表` 和 `营运报表`。

### 已检查

- 已于 2026-06-10 将商户 HQ 文档与已部署的 HQ 商户后台核对，并排除 X1 Control Plane／内部合作伙伴管理功能，不加入公开用户文档。
- 重新截取并确认 HQ 截图已等待页面载入完成，确保发布图片显示实际内容，而不是载入画面。

## 2026-05-27（v1.7.16 POS 重新发布、HQ 修复和 HQ Agent 复查）

### 已更新

- `设置` 和 `功能与特性地图`：补充模板字体设定现在包含 `八达通增值单` 和 `入账 / 出账单`，这些单据可按已保存的字号设定输出。
- `连接 AI 助理`：将 ChatGPT 设置示例与 HQ Agent 提交资料中的 `X1 HQ Agent` app 名称对齐。

### 截图更新

- 新增待补截图，要求显示 `设置` -> `打印` -> `模板` 选择器中的 `八达通增值单` 和 `入账 / 出账单`。

### 已检查

- 已检查 POS `v1.7.16` 重新发布说明，以及 2026-05-21 文档基线后的源码变更，包括首次/后台同步稳定性、结账与作废崩溃修复、转台修复、打印机超时/重试行为、模板字体路由、考勤结果提示、报表查询修复、Android 同步上传、Mac 相机扫描器设置和浏览器 POS 预览修复。
- 已检查 HQ 在上一轮还原流程文档更新后的变更，包括折扣规则编辑器保存响应和折扣序列化字段规范化。确认这些是既有折扣步骤的可靠性修复，不是新的管理员流程。
- 已检查 HQ Agent 的 ChatGPT app 提交资料、MCP CORS 处理和工具注解。确认面向用户的文档影响只有上述 app 名称措辞；工具注解和 CORS 细节不新增管理员步骤。
- 确认更广泛的 POS 浏览器沙盒、崩溃日志分析、支持日志、部署/runbook 和内部查询追踪更新，不需要在线文档新增收银员/管理员步骤。
...
- `管理打印机`：新增 HQ 管理打印机设置流程，并说明打印队列错误任务会触发打印机警示标记，直到 `Error` 队列清空。
- `我的订阅` 和 `首次初始化`：说明 POS 设备名称现在会在启用/验证订阅时记录，并说明删除 POS 店铺配置前会释放相关 X1 POS 订阅席位。
...
- 已检查 `v1.7.12` 到 `v1.7.13` 的 POS 源码差异，包括发布说明、UI 标签、设备警示实现、订阅启用/验证变更，以及删除店铺配置行为。
...
- `配置付款设备` 和 `设备`：补充 Octopus V3 Reader `设备编号` 栏位和保存要求。
- 新增功能指南：订单功能菜单中的 `八达通结算`。
- `销售流程` 和 `功能与模块索引`：将 `八达通结算` 加入订单功能说明。

### 截图更新

- 新增标签自适应文字大小和 Octopus settlement 的待补截图。
- 更新 Octopus V3 Reader 截图需求，加入 `设备编号`。

### 已复查

- 已检查从上一版 POS 手册基准到 `v1.7.11` 的 POS 源码变化，包括发布说明和源码差异。
- 已确认 ODO 结账状态同步、后台 ODO 上传、外带导航上下文、Octopus 增值单标题字号、结算响应性，以及稳定性／崩溃处理属于行为修复或可靠性改进，不需要在上述更新之外新增收银员步骤。

## 2026-04-25（HQ POS 使用者批量建立和工作区存取）

### 更新内容

- `POS Users`：补充在同一个 `New POS User` 窗口使用 `Beast Mode` 建立多个 POS 使用者的说明，包括每行栏位，以及共用 `User Groups`、`Shop`、登入方式和状态设定。
- `Workspace Administration`：补充 `Workspace admin` 存取状态说明，并说明工作区管理员即使没有直接公司、品牌或店铺分配，也已拥有当前工作区的存取权限。
...
- `Menu Items`：补充抽屉标题中的当前记录信息、`Skip to Next` / `Update & Next` 会保留当前分页的流程，以及各店铺启用状态相关的店铺设定文字。
...
- `Categories`：补充 `Category Code`、分类 `Shop Settings`，以及用共用设定一次建立多个分类的 `猛兽模式` 说明。
- `Smart Categories`：补充用共用设定一次建立多个智能分类的 `猛兽模式` 说明。
...
## 2026-04-17（HQ 导航与帐单调整）

### 更新内容

- `市集与帐单`：将 HQ 文档章节名称调整为与后台侧边栏一致。
- `My Subscriptions`：将进入路径更新为顶层 `My Subscriptions` 侧边栏项目。
- `Billing`：将进入路径更新为顶层 `Billing` 侧边栏项目，并补充自订线下合约帐单说明。
...
- 已比对 HQ 后台中订阅、帐单、促销和折扣页面的导航变更与手册基准。
- 已确认本次只调整导航标签与帐单文字，不需要新增截图。
