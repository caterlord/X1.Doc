...
## 2026-08-09（POS v1.8.0 与 HQ 公开商户后台对齐）

### 已更新

- `报表`：补充 v1.8.0 仪表板卡片、柱状图与环形图、图表数值查看，以及 `DETAILS` 明细表。
- `设备` 和 `管理打印机`：将打印队列分页对齐为 `队列`、`历史` 和 `需要处理`；补充打印预览、缩放／平移、安全重试、重复打印风险提示、`标记为已打印`、`取消` 及任务详情。
- `付款方式`：补充 `猛兽模式` 每批最多 100 行，以及整批验证失败时不会建立部分资料的说明。
- `菜单项目`：补充每家门店的 `外带附加费` 和可展开的 `其他设置`。
- `智能分类`、`线上分类` 和 `线上菜单`：补充 `已发布`、`公开显示`、`显示未发布`、`显示于 ODO`，以及新门店映射的默认发布状态。
- `第三方平台`：补充 Foodpanda 的商户优先表单、可搜寻 ID 选择器、`无法使用的 ID（{id}）`、项目选择器、保存后同步要求，以及手动 `菜单同步`。
- `折扣`：补充标准 `折扣：百分比` 保留价格倍率和 `套用到商品层级` 行为。

### 截图更新

- 新增 POS 仪表板图表、打印预览和 `需要处理` 动作的待补截图。
- 新增 HQ 外带附加费、ODO 发布状态、折扣百分比输入、线上菜单门店时段、线上分类发布状态和 Foodpanda 菜单同步的待补截图。
- 保留 MeiTuan Webhook 截图要求，并与新的第三方平台指引连结。

### 已检查

- 已将 POS tag `v1.8.0`（`ce1b6414`）与 `v1.7.19..v1.8.0` 源码范围对照；打印机流程及报表仪表板是需要新增公开步骤的变更。
- 已检查 PAX 拆分／合并稳定性、账单折扣重新验证，以及 POS 的运行环境、网络、诊断、崩溃和封装变更；这些不新增现有指引以外的公开恢复步骤。
- 已检查 HQ 2026-07-18 基线后至 `8ae01e6` 的商户功能变更，包括付款方式、门店附加费、线上发布、Foodpanda／菜单同步和折扣设置。
- 排除 X1 Control Plane、内部管理、合作伙伴、seed／修复／默认资料和数据规范化变更，不加入公开操作文档。

## 2026-07-19（POS v1.7.19 与 HQ 正式环境对齐）

### 已更新

- `设置`：补充 `付款后保留餐台`、已付款订单查看与调整、余额结清，以及 `释放餐台`。
- `报表`：补充 `14. 整单取消报表`，包括操作人员、取消原因、项目明细和原项目总额查看。
- `加购项` 和 `套餐群组`：补充新的 `使用此群组的菜品` 页签，以及检查或更改父菜品连结的关联操作。
- `促销`：补充 `保存并继续`、`保存并关闭`，以及规则编辑器会记住上次保存操作的行为。
- `第三方平台`：补充恢复的 `MeiTuan` Webhook 字段，包括 `API URL`、`Shop ID`、机密 Token 字段和 `Token 到期日期/时间`。

### 截图更新

- 更新 POS 报表待补截图，要求显示 `14. 整单取消报表` 及其稽核栏位。
- 新增已付款餐台设置与释放操作、群组使用页签、促销保存菜单和 MeiTuan Webhook 设置的待补截图。

### 已检查

- 确认上一版已部署文档基线为 2026-06-29；POS 发布说明上次完整检查至 `v1.7.16`，后续订位指引已经纳入文档。
- 已检查 POS `v1.7.17`、`v1.7.18` 和 `v1.7.19`。最新源码发布为 `v1.7.19`，发布于 2026-07-18。
- 已将 HQ 商户后台与 2026-07-18 更新的正式环境核对，并检查截至该次部署的商户功能源码变更；X1 Control Plane／内部合作伙伴管理功能继续排除在公开文档之外。
- 确认其余已检查的 POS 和 HQ 变更属于可靠性、同步、诊断、封装、预设资料或数据规范化修复，不新增操作人员或商户管理员步骤。
