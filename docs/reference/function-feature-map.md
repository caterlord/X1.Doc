...
| Devices | Printer add/edit/delete, info/status, test print, spooler queue/history/needs-attention actions, print preview, safe and duplicate-risk retry controls, failed-job alert badges, automatic retry details, HQ-managed printer setup status, advanced printer output settings, payment/input/X1 device configuration (including scanner input source selection, Linkly EFTPOS pairing, and Octopus V3 `Device ID`) | [Screen Guide: Devices](../client/devices), [Function: Manage Printers](../functions/manage-printers), [Function: Configure Payment Devices](../functions/configure-payment-devices), [Function: Configure Input Devices](../functions/configure-input-devices), [Function: Configure X1 Products](../functions/configure-x1-products), [Troubleshooting](./troubleshooting) |
| Reports | Report selection, date filters, dashboard summary cards, bar/donut chart inspection, `DETAILS` tables, print output, `Voided Order Report` audit details | [Screen Guide: Reports](../client/reports) |
| Settings | Language configuration, item-name mapping (`Main`/`Alt`), login quick-switch enablement, POS code, cash register code, terminal `Mode` (`Restaurant` / `Quick Order`), general order-flow toggles (`Enable Dine In Auto Open Order`, `Enable Takeaway Auto Open Order`, `Enable Default Takeaway Mode`, optional auto-assign table code), paid-table workflow (`Keep Table Active After Payment`, balance settlement, `Release Table`), `Clearing` day-end controls (`Use Starting Day Section`, unclosed-order checks, pre-new-workday processing) plus `Local Transaction Retention (Days)`, restore-from-backup action, rounding controls (`Rounding Method`, `Decimal Places`, `Round For Cash Only`), printing controls (`Mapping`, `Setting`, `Template`), `Auto Print Receipt`, order-contact print toggles, receipt `Customer Contact` font section, template font profiles including `Octopus Add Value Slip` and `Pay In/Out Slip`, label `Enable adaptive text size`, `Show AUTO REPRINT label on automatic retries`, print language override, report print quantity, `Daily Report Sections`, advanced day-end report section toggles, third-party integrations (enabled-only list), ODO channel mapping (mapped-only list) | [Screen Guide: Settings](../client/settings), [Troubleshooting](./troubleshooting) |
